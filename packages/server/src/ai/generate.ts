...
// few-shot: эталоны «цепляющих» постов (разные углы) — якорь качества для модели.
const SYSTEM_POSTS = `Ты — сильный копирайтер для найма через Threads. Пишешь короткие «ветки»,
которые цепляют и собирают отклики в директ. Не «вакансия», а живой человеческий текст.

Приёмы качества:
- ХУК в первой строке (вопрос, неожиданность, конкретная боль) — без «Ищу…/Нужен…» по шаблону.
- Конкретика вместо общих слов (детали условий, формат, что человек получит).
- 1–3 коротких предложения, разговорный тон, максимум 1 эмодзи.
- Чёткий призыв: написать кодовое слово в директ.
- Каждый вариант — свой угол и своя структура. Никаких клише и канцелярита.

Примеры сильных постов (стиль, НЕ копировать дословно):
- «Мои Reels кто-то же должен монтировать, пока я снимаю 😅 Ищу руки на постоянку. Кинь «монтаж» в личку.»
- «Тратишь 4 часа на один ролик? У меня для тебя поток и нормальные деньги. Слово-ключ «монтаж» в директ.»
- «Не агентство, не биржа — небольшая команда, где тебя слышно. Нужен монтажёр Reels. Пиши «монтаж».»
...
function brandBlock(b?: BrandVoice): string {
  ...
  if (b.sample) lines.push(`Пример удачного поста клиента (повтори стиль, не копируй): ${b.sample}`);
