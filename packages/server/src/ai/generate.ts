...
const SYSTEM_REPLIES = `Ты пишешь авто-ответы для директа Threads при найме. Кандидат прислал кодовое слово — бот отвечает этим текстом ОДИН раз.
ЦЕЛЬ ответа — увести кандидата на указанный КОНТАКТ (обычно Telegram-ник), где ему пришлют тестовое и следующие шаги.
Правила:
- Тепло, коротко (1–2 предложения), по-человечески.
- ОБЯЗАТЕЛЬНО укажи КУДА писать — контакт из «Куда перенаправляем» (например, Telegram @ник). Дай понять, что ИМЕННО ТАМ пришлют детали/тестовое.
- Можно мягко попросить продублировать кодовое слово этому контакту.
- НИКОГДА не обещай «ждать в директе», «пришлём здесь», «жди сообщение в директе», «ответим в директе» — следующий шаг ТОЛЬКО на указанном контакте.
- Если контакт не задан — попроси оставить удобную связь (Telegram).
Варианты отличаются формулировками, но все ведут на контакт. Верни СТРОГО JSON-массив строк, без markdown.`;
      ...
      messages: [
        {
          ...
          content:
            ...
            `Куда перенаправляем кандидата (контакт): ${input.redirectTarget || '— (не задан, попроси оставить Telegram)'}\n` +
            (input.brief ? `Доп. указания: ${input.brief}\n` : '') +
            (input.seed ? `СДЕЛАЙ ВАРИАНТЫ В ДУХЕ ЭТОГО ОТВЕТА (сохрани смысл и шаг — увести на контакт; перефразируй, поменяй тон):\n«${input.seed}»\n` : '') +
            brandBlock(input.brand) +
